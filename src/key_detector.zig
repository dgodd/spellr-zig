...
const NUM_CLASSES = 8;
const KEY_HEURISTIC_WEIGHT: f64 = 1e5; // 10^key_heuristic_weight (default 5)
...
fn extractFeatures(s: []const u8) Features {
    ...
    const lfd_plus = if (flen > 0) @abs(@as(f64, @floatFromInt(cnt_plus)) / flen - ideal_freq / flen) else 0.0;
    const lfd_minus = if (flen > 0) @abs(@as(f64, @floatFromInt(cnt_minus)) / flen - ideal_freq / flen) else 0.0;
    const lfd_under = if (flen > 0) @abs(@as(f64, @floatFromInt(cnt_under)) / flen - ideal_freq / flen) else 0.0;
    const lfd_slash = if (flen > 0) @abs(@as(f64, @floatFromInt(cnt_slash)) / flen - ideal_freq / flen) else 0.0;
    const lfd_A = if (flen > 0) @abs(@as(f64, @floatFromInt(cnt_A)) / flen - ideal_freq / flen) else 0.0;
    const lfd_z = if (flen > 0) @abs(@as(f64, @floatFromInt(cnt_z)) / flen - ideal_freq / flen) else 0.0;
    const lfd_Z = if (flen > 0) @abs(@as(f64, @floatFromInt(cnt_Z)) / flen - ideal_freq / flen) else 0.0;
    const lfd_q = if (flen > 0) @abs(@as(f64, @floatFromInt(cnt_q)) / flen - ideal_freq / flen) else 0.0;
    const lfd_Q = if (flen > 0) @abs(@as(f64, @floatFromInt(cnt_Q)) / flen - ideal_freq / flen) else 0.0;
    const lfd_X = if (flen > 0) @abs(@as(f64, @floatFromInt(cnt_X)) / flen - ideal_freq / flen) else 0.0;
    const lfd_x = if (flen > 0) @abs(@as(f64, @floatFromInt(cnt_x)) / flen - ideal_freq / flen) else 0.0;
        ...
        switch (kind) {
            .title => {
                if (i < s.len and s[i] >= 'A' and s[i] <= 'Z') {
                    ...
                    if (i > start + 1) lengths[n] = i - start else i = start + 1;
                } else { i += 1; continue; }
            },
            .lower_chunk => {
                if (i < s.len and s[i] >= 'a' and s[i] <= 'z') {
...
fn classLogProb(features: Features, class_idx: usize) f64 {
    ...
    // apply heuristic weight boost to key classes
    if (isKeyClass(class_idx)) log_prob += @log(KEY_HEURISTIC_WEIGHT);
    return log_prob;
}
...
pub fn isKey(s: []const u8) bool {
    ...
    for (0..NUM_CLASSES) |i| {
        const p = classLogProb(features, i);
